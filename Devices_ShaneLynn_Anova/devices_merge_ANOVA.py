#All credit to Shane Lynn
#This was an amazing tutorial for using joins/merges
#Previously only used joins before in SQL
#All data sets can be gleaned from shanelynn.com

#1 Major Difference: Decided to run a one-way ANOVA because the dataset
            #had some interesting questions that could be answered by ANOVA
...
on = 'use_id', how = 'left') #merge of the left dataset, right dataset responsible for proviving null columns
user_device.head()

user_device2 = users.join(device[['use_id', 'device']], on = 'use_id', lsuffix='users', rsuffix = 'device')
user_device2.head()
#the diff between merge and join, the 'joining' column is repeated in the join, so 2 use_id columns in the join table
#seems better to use merge for a cleaner table
...
##Let's get a summary of the target column 'out-going minutes', but first let's make life easier and rename wordy columns
